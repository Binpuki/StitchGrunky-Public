...
class EditorPlayState extends MusicBeatSubstate
{
	...
	public function new(playbackRate:Float)
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('yakuza'));
		...
		scoreTxt.setFormat(Paths.font("neon.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		dataTxt.setFormat(Paths.font("neon.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		dataTxt.scrollFactor.set();
		dataTxt.borderSize = 1.25;
		add(dataTxt);

		var tipText:FlxText = new FlxText(10, FlxG.height - 24, 0, 'Press ESC to Go Back to Chart Editor', 16);
		tipText.setFormat(Paths.font("neon.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		if(notes.length > 0)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (visualSongPosition - daNote.strumTime > noteKillOffset)
				{
		...
		var time:Float = CoolUtil.floorDecimal((visualSongPosition - ClientPrefs.data.noteOffset) / 1000, 1);
	...
	function generateSong(dataPath:String)
	{
		...
		FlxG.sound.music.loadEmbedded(Paths.inst(songData.song));
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, gottaHitNote ? playerStrums : opponentStrums, oldNote, this);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus + 1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote), daNoteData, gottaHitNote ? playerStrums : opponentStrums, oldNote, true, this);
