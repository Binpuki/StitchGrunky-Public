	...
	**/
	...
	override function create()
	{
		...
		for (i in 0...tipTextArray.length) {
			...
			tipText.y += i * 12;
			tipText.setFormat(Paths.font("neon.ttf"), 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		for (i in 0...directories.length) {
			...
			CoolUtil.recursiveSearch(directory, function(filePath:String){
				if (filePath.endsWith('.json')) {
					var stageToCheck:String = filePath.substr(filePath.lastIndexOf("/") + 1, filePath.length - filePath.lastIndexOf("/") - 6);
	...
	function addEventsUI():Void
	{
		...
		for (i in 0...directories.length) {
			var directory:String =  directories[i];
			CoolUtil.recursiveSearch(directory, function(filePath:String){
				if (filePath != 'readme.txt' && filePath.endsWith('.txt')) {
					var fileToCheck:String = filePath.substr(filePath.lastIndexOf("/") + 1, filePath.length - filePath.lastIndexOf("/") - 5);
					if(!eventPushedMap.exists(fileToCheck)) {
						eventPushedMap.set(fileToCheck, true);
						eventStuff.push([fileToCheck, File.getContent(filePath)]);
					}
				}
			});
		}
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("neon.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font("neon.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	function loadJson(song:String):Void
	{
		...
		catch(e)
		{
			...
			if(missingText == null)
			{
				...
				missingText.setFormat(Paths.font("neon.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
