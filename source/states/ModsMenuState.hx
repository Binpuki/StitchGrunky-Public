...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(modsList.all.length < 1)
		{
			...
			noModsTxt.setFormat(Paths.font("neon.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			noModsTxt.borderSize = 2;
			add(noModsTxt);
			noModsTxt.screenCenter(Y);

			var txt = new FlxText(bgList.x + 15, bgList.y + 15, bgList.width - 30, "No Mods found.", 16);
			txt.setFormat(Paths.font("neon.ttf"), 16, FlxColor.WHITE);
			add(txt);
		...
		modDesc.setFormat(Paths.font("neon.ttf"), 24, FlxColor.WHITE, LEFT);
		add(modDesc);
		...
		modRestartText.setFormat(Paths.font("neon.ttf"), 16, FlxColor.WHITE, RIGHT);
		add(modRestartText);
...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String)
	{
		...
		text.setFormat(Paths.font("neon.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		text.borderSize = 2;
		text.y -= Std.int(text.height / 2);
		add(text);

		var isPixel = false;
		var bmp = Paths.cacheBitmap(Paths.mods('$folder/pack.webp'));
		if(bmp == null)
		{
			bmp = Paths.cacheBitmap(Paths.mods('$folder/pack-pixel.webp'));
			isPixel = true;
		}
