...
class PlayState extends MusicBeatState
{
	...
	public var maxScore:UInt = 1000000;
	public var songScore(get, null):UInt;
	private function get_songScore():UInt
	{
		if (ratingsData[0].hits == totalNotes)
			return maxScore;
		
		var score:UInt = 0;
		for (i in 0...ratingsData.length)
			score += Std.int((maxScore / totalNotes) * ratingsData[i].hits * ratingsData[i].scoreMult);
	...
	override public function create()
	{
		...
		//trace(Assets.getLibrary("").list("IMAGE"));
		...
		switch (curStage)
		{
			case "stage": new states.stages.Default(); // Default
		}
		...
		timeTxt.setFormat(Paths.font("neon.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		// god maybe put the custom event initialization BEFORE the notes are generated!?!?!?! - ur local vivi binpuki
		...
		scoreTxt = new FlxText(0, healthBar.y + (40 * (2 / 3)), FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("neon.ttf"), Std.int(20 * (2 / 3)), FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("neon.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		Application.current.window.onKeyUp.add(onKeyRelease);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		try 
		{
			FlxG.sound.music.loadEmbedded(Paths.inst(songData.song), false, true, finishSong.bind());
		}
		catch(e:Dynamic) {}
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, gottaHitNote ? playerStrums : opponentStrums, oldNote);
				swagNote.mustPress = gottaHitNote;
				swagNote.sustainLength = songNotes[2];
				swagNote.playbackRate = playbackRate;
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus + 1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote), daNoteData, gottaHitNote ? playerStrums : opponentStrums, oldNote, true);
						sustainNote.mustPress = gottaHitNote;
						sustainNote.gfNote = (section.gfSection && (songNotes[1]<4));
						sustainNote.noteType = swagNote.noteType;
						sustainNote.scrollFactor.set();
						sustainNote.parent = swagNote;
						sustainNote.playbackRate = playbackRate;
	...
	function makeEvent(event:Array<Dynamic>, i:Int)
	{
		...
		var eventName:String = Std.string(event[1][i][0]);
		#if LUA_ALLOWED
		startLuasNamed('custom_events/' + eventName + '.lua');
		#end

		#if HSCRIPT_ALLOWED
		startHScriptsNamed('custom_events/' + eventName + '.hx');
		#end
	...
	{
		var isOpp:Bool = player != 1;

		var spacing:Float = !ClientPrefs.data.middleScroll ? ((FlxG.width * 0.75) / 2) : 0;
		var strumLineX:Float = STRUM_X + (spacing * player) + (ClientPrefs.data.middleScroll && !isOpp ? 460 - (Note.swagWidth * 2) : 0);
	...
	{
		var mult:Float = FlxMath.lerp(2 / 3, iconP1.scale.x, FlxMath.bound(1 - (elapsed * 9 * playbackRate), 0, 1));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(2 / 3, iconP2.scale.x, FlxMath.bound(1 - (elapsed * 9 * playbackRate), 0, 1));
	...
	// HUD Overlays that aren't over the entire HUD
	...
	private function cachePopUpScore()
	{
		...
		for (rating in ratingsData)
			Paths.image("ui/" + uiPrefix + "judge/" + rating.image + uiSuffix);
		for (i in 0...10)
			Paths.image("ui/" + uiPrefix + 'combo/num' + i + uiSuffix);
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		comboDisplay.updateCombo(daRating.image, combo, noteDiff);
	}

	public var strumsBlocked:Array<Bool> = [];
	private var keysNotReleased:Array<Bool> = [];
	private function onKeyPress(key:KeyCode, modifier:KeyModifier):Void
	{
		var eventKey:FlxKey = Converter.fromKeyCodeToFlxKey(key, modifier);
		var key:Int = getKeyFromEvent(keysArray, eventKey);

		if (!controls.controllerMode && !keysNotReleased[key])
			keyPressed(key);

		keysNotReleased[key] = true;
	}
	...
	private function onKeyRelease(key:KeyCode, modifier:KeyModifier):Void
	{
		var eventKey:FlxKey = Converter.fromKeyCodeToFlxKey(key, modifier);
		var key:Int = getKeyFromEvent(keysArray, eventKey);

		if(!controls.controllerMode && key >= 0) 
			keyReleased(key);

		keysNotReleased[key] = false;
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		vocals.volume = 1;
		strumPlayAnim(true, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate, note.isSustainNote, note.isSustainNote && note.animation.curAnim.name.endsWith("holdend"));
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		else strumPlayAnim(false, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate, note.isSustainNote, note.isSustainNote && note.animation.curAnim.name.endsWith("holdend"));
		vocals.volume = 1;
	...
	override function destroy() {
		...
		Application.current.window.onKeyUp.remove(onKeyRelease);
		FlxG.animationTimeScale = 1;
	...
	{
		if(FlxG.sound.music.time >= -ClientPrefs.data.noteOffset && !windowFocused)
		{
			if (Math.abs(FlxG.sound.music.time - (visualSongPosition - Conductor.offset)) > (20 * playbackRate)
				|| (SONG.needsVoices && Math.abs(vocals.time - (visualSongPosition - Conductor.offset)) > (20 * playbackRate)))
			{
				resyncVocals();
			}
		}
	...
	public function initHScript(file:String)
	{
		try
		{
			...
			{
				trace(newScript.parsingException.message);
	...
	function strumPlayAnim(isDad:Bool, id:Int, time:Float, isSusNote:Bool, isSusEnd:Bool) {
