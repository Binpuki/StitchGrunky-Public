...
class Paths
{
	...
	static public function image(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxGraphic
	{
		...
		if (currentTrackedAssets.exists(file))
		{
			...
			bitmap = WebP.getBitmapData(file);
		else
		#end
		{
			file = getPath('images/$key.webp', IMAGE, library);
			if (currentTrackedAssets.exists(file))
			{
				localTrackedAssets.push(file);
				return currentTrackedAssets.get(file);
			...
			{
				bitmap = WebP.getBitmapDataFromBytes(OpenFlAssets.getBytes(file));
			}
		}
	...
	inline static public function modsImages(key:String) {
		return modFolders('images/' + key + '.webp');
	}
	...
	public static function loadAnimateAtlas(spr:FlxAnimate, folderOrImg:Dynamic, spriteJson:Dynamic = null, animationJson:Dynamic = null)
	{
		...
		if(Std.isOfType(folderOrImg, String))
		{
			...
			for (i in 0...10)
			{
				...
				else if(Paths.fileExists('images/$originalPath/spritemap$st.webp', IMAGE))
				{
