...
class Conductor
{
	private static var timeOffset:Float = 0;
	...
	private static function get_songPosition():Float
		return (#if sys Sys.time() #else Date.now().getTime() / 1000 #end - timeOffset) * 1000 * playbackRate;

	private static function set_songPosition(value:Float):Float
		return timeOffset = Sys.time() - (value / 1000);
