...
class Note extends FlxSprite
{
		...
		if(noteData > -1 && noteType != value) {
			switch(value) {
				case 'Hurt Note':
					...
					//this used to change the note texture to HURTNOTE_assets.webp,
	...
	public function new(strumTime:Float, noteData:Int, strumGroup:FlxTypedGroup<StrumNote>, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false, ?createdFrom:Dynamic = null)
	{
		super();
	...
	public function reloadNote(texture:String = '', postfix:String = '') {
		...
		if(customSkin == _lastValidChecked || Paths.fileExists('images/' + path + customSkin + '.webp', IMAGE))
		{
			skin = customSkin;
	...
	override function update(elapsed:Float)
	{
		...
		if (strumGroup != null)
		{
			var strum:StrumNote = strumGroup.members[noteData];
			followStrumNote(strum, (60 / PlayState.SONG.bpm) * 1000, PlayState.SONG.speed / playbackRate);
			
			if (isSustainNote && strum.sustainReduce) 
				clipToStrumNote(strum);
		}
